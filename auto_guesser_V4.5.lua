--[[
    ╔══════════════════════════════════════════════════════════════════╗
    ║     AMETHYST HUB  ·  Hot or Cold  ·  Semantic AI v4.0           ║
...
local function applyTheme()
    ...
    Window.Content.BackgroundColor3 = t.bg
    for _, frame in ipairs(Window.Content:GetDescendants()) do
        if frame:IsA("Frame") and frame:GetAttribute("Themed") then
            if frame:GetAttribute("PanelType") == "panel" then
                frame.BackgroundColor3 = t.panel
            elseif frame:GetAttribute("PanelType") == "accent" then
                frame.BackgroundColor3 = t.accent
            elseif frame:GetAttribute("PanelType") == "danger" then
                frame.BackgroundColor3 = t.danger
            elseif frame:GetAttribute("PanelType") == "success" then
                frame.BackgroundColor3 = t.success
            elseif frame:GetAttribute("PanelType") == "hot" then
                frame.BackgroundColor3 = t.hot
            end
        end
        if frame:IsA("TextLabel") or frame:IsA("TextButton") then
            if frame:GetAttribute("TextType") == "primary" then
                frame.TextColor3 = t.text
            elseif frame:GetAttribute("TextType") == "sub" then
                frame.TextColor3 = t.subtext
            elseif frame:GetAttribute("TextType") == "accent" then
                frame.TextColor3 = t.accent
            end
        end
    end
    if Window.Gradient then
...
-- Build lookup tables
local wordToCluster, allWords, wordSet = {}, {}, {}
local wordSeen = {}
...
Window.Gradient = WindowGradient
Window.BG = Window  -- for theme access
