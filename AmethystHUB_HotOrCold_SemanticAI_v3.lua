--[[
    ╔══════════════════════════════════════════════════════════════════╗
    ║   🔮 AMETHYST HUB  ·  Hot or Cold – Semantic AI v3.0            ║
    ║   Ultra Heat Engine · Multiplier Aware · Collapsible UI          ║
    ╠══════════════════════════════════════════════════════════════════╣
    ║  UPGRADES OVER v2:                                               ║
    ║  • Deep-scans "word xN" multipliers from leaderboard rows        ║
    ║  • Heat = 1,000,000 ÷ rank  (lowest # = exponentially more heat) ║
    ║  • x2/x3 multipliers give ×1.6 / ×2.2 cluster heat bonus        ║
    ║  • Persistent cluster heat accumulates across ticks (80% decay)  ║
    ║  • Cluster-confidence rank: fires entire hot cluster first        ║
    ║  • Open / Close menu toggle button on header                     ║
    ║  • Warmup seeds with statistically top game-answer words first   ║
    ║  • Broader GUI scan catches words not in strict sibling layout   ║
    ║  • "Hottest" = lowest # rank (best position, most heat)          ║
    ╚══════════════════════════════════════════════════════════════════╝
--]]
...
--  Words tried first when no leaderboard data is available.
--  Ordered by statistical frequency in Roblox "Guess the Word / Hot or Cold" games.
local WARMUP_PRIORITY = {
    -- Core food (most common answer category)
...
local allWords      = {}
local wordSeen      = {}
local wordSet       = {}   -- fast O(1) membership check
...
-- ─── WARMUP LIST (ordered by priority, filtered to words in CLUSTERS) ─────────
local warmupList = {}
local warmupSeen = {}
...
-- Append remaining cluster words not in warmup list
...
-- Extract word + multiplier from a TextLabel string
-- Handles: "food", "food x2", "macaroni  x2", "pasta x3", "polar bear x2"
local function extractWordAndMult(text)
    if not text or text == "" then return nil, 1 end
    local t = string.lower(string.gsub(text, "%s+", " "))
    t = string.gsub(t, "^%s+", "")
    t = string.gsub(t, "%s+$", "")
    -- "word xN" or "multi word xN"
    ...
    -- Plain alphabetic / multi-word (no multiplier)
...
-- Deep scan PlayerGui for all {word, rank, mult} triples on leaderboard
-- Strategy: find every visible "#N" label, then search sibling/nearby labels for the word
local function scanAllRankedWords()
        ...
        if el:IsA("TextLabel") and el.Visible then
            ...
            if rankStr then
                ...
                if parent then
                    for _, sib in ipairs(parent:GetChildren()) do
                        if sib:IsA("TextLabel") and sib ~= el and sib.Visible then
                            ...
                            if word and not seen[word] then
                                -- Accept if exact match in cluster, or a single clean word
                    ...
                    -- Also search parent's parent (one level up) for cross-frame layouts
    ...
    -- Sort: lowest rank number first (best position = most heat)
    table.sort(results, function(a, b) return a.rank < b.rank end)
    return results
end

-- Compute per-cluster accumulated heat from the live leaderboard snapshot
-- Heat formula: 1,000,000 / rank  × multiplier bonus
-- → rank #1 = 1,000,000 pts; rank #10 = 100,000; rank #100 = 10,000; rank #37793 ≈ 26
-- Multiplier bonus: x1 = ×1.0; x2 = ×1.6; x3 = ×2.2; x4 = ×2.8 ...
...
-- Score a single candidate using accumulated cluster heat
...
local function pickNextWord()
    if not S.SmartGuessing then
        -- Sequential scan over priority-ordered warmup list
    ...
    if #leaderboard == 0 then
        -- No board data yet: use warmup priority
        for _, word in ipairs(warmupList) do
            if not S.Guessed[word] then
    ...
    -- If no heat at all, diversify (fallback sequential)
...
TitleLbl.Text               = "🔮  AMETHYST HUB  ·  Semantic AI v3.0"
...
StartBtn.MouseButton1Click:Connect(function()
    ...
    task.spawn(function()
        while S.Running and not S.KillSwitch and _G.AmethystHub_CurrentGen == GEN_ID do
            ...
            -- Jitter ±40 ms to avoid pattern detection
...
StopBtn.MouseButton1Click:Connect(function()
    S.KillSwitch = true
    S.Running    = false
    -- Reset persistent heat on halt so next run starts fresh
    clusterHeat  = {}
